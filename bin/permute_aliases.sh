#!/usr/bin/env bash
# Prints all permutation of a given program as bash alias commands to stdout.
# Usage: ./permute_aliases.sh <program> >.bash_aliases_<program>
# source the generated file in you bash configuration file(s).

# In subject, swap subject[a_pos] with subject[b_pos].
swap() {
	local subject="$1"
	local a_pos="$2"
	local b_pos="$3"

	a="${subject:${a_pos}:1}"
	b="${subject:${b_pos}:1}"
	a_pos=$((a_pos + 1))
	b_pos=$((b_pos + 1))
	permuted="$(echo "$subject" | sed -e "s/./${b}/${a_pos}" | sed -e "s/./${a}/${b_pos}")"
	echo "$permuted"
}

print_alias() {
	local program="$1"
	local alias="$2"

	printf "alias %s='%s'\n" "$alias" "$program"
}

# Generate permutation aliases recursively.
permutate() {
	local program="$1"
	local subject="$2"
	local pos="$3"

	local i=$((pos+1))
	if [ "$i" -lt "${#subject}" ]; then
		permutate "$program"  "$subject" $i
	fi
	for ((; $i < ${#subject}; i++)); do
		local permuted=$(swap "$subject" "$pos" "$i")
		print_alias "$program"  "$permuted"
		nextpos=$((pos+1))
		permutate "$program" "$permuted" $nextpos
	done
}

if [ "$#" -ne "1" ]; then
	echo "One argument expected: string to permute." 1>&2
	exit 1
else
	program="$1"
	echo "# Below follows all permutation aliases of \"${program}\", generated by \"${0##*/}\"."
	permutate "$program" "$program" 0
fi
